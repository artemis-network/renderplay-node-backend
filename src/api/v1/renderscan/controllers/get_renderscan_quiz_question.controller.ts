...
import {
	getRenderScanQuizByContestId, getRenderScanCurrentLiveQuestion, doesGameCompleted
...
export const getRenderScanQuestionController = async (req: Request, res: Response) => {
	...
	if (question === null) return res.status(200).json({ expiresAt: quiz.lobbyExpiresAt, isGameStarted: false, isGameEnded: isGameEnded })
